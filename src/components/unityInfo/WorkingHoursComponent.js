...
import { WORKING_HOURS_STRINGS } from "../../languages/index"
import UnityShiftComponent from "./UnityShiftComponent"

export default class WorkingHoursComponent extends PureComponent {
    ...
    mountList() {
        let filteredServices = this.state.workingHours.filter((option, index, self) =>
            self.findIndex(t =>
        ...
        let listHourGroups = this.state.workingHours.map(hourGroup => {
            let workingDays = []
            ...
            this.state.workingHours.length > 0 ?
                <View style = { this.stylesView.general } accessibilityLabel = "viewGeneral">
                    <Text style = { this.stylesText.title } accessibilityLabel = "textTitle">
                        { WORKING_HOURS_STRINGS.workingHours }
                    </Text>
                    { this.renderTabs() }
                </View>
            : null
